...
export const detectVisitorLocation = async (): Promise<GeolocationData> => {
  try {
    ...
    const cachedLocation = localStorage.getItem('Mandiri_visitor_location')
    const cachedTimestamp = localStorage.getItem('Mandiri_location_timestamp')
    ...
    // Cache the result
    localStorage.setItem('Mandiri_visitor_location', JSON.stringify(locationData))
    localStorage.setItem('Mandiri_location_timestamp', Date.now().toString())
...
export const setLanguagePreferenceByLocation = async (): Promise<'id' | 'en' | 'ar' | 'zh' | 'ja' | 'es' | 'fr' | 'ko'> => {
  try {
    ...
    localStorage.setItem('Mandiri_lang_preference', langPreference)
    ...
    // Default to English
    localStorage.setItem('Mandiri_lang_preference', 'en')
    return 'en'
  }
}
